...
export default function ReportIssueForm() {
  ...
  const onSubmit = (values: IssueFormValues) => {
    // Mock submit
    toast.success(t("toast.submitted.title"), {
      description: t("toast.submitted.desc"),
    });
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">{t("form.page_title")}</h1>
        <div className="text-sm text-muted-foreground">{t("form.step")} {step + 1} of {steps.length} - {t(stepKeys[step])}</div>
      </div>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="details" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} className="space-y-4">
            ...
            <div>
              ...
              <select className="mt-1 w-full rounded-md border bg-background px-3 py-2" {...form.register("category")}>
                <option value="pothole">{t("form.cat.pothole")}</option>
                <option value="garbage">{t("form.cat.garbage")}</option>
                <option value="streetlight">{t("form.cat.streetlight")}</option>
                <option value="water">{t("form.cat.water")}</option>
                <option value="other">{t("form.cat.other")}</option>
              </select>
            </div>
            <div>
        ...
        {step === 2 && (
          <motion.div key="photos" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">{t("form.attach_hint")}</p>
          </motion.div>
        )}

        {step === 3 && (
          <motion.div key="review" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} className="space-y-3">
            <h2 className="text-lg font-medium">{t("form.review_label")}</h2>
      ...
      <div className="flex items-center justify-between gap-2">
        <button type="button" onClick={prev} disabled={step===0} className="rounded-md border px-4 py-2 disabled:opacity-50">{t("btn.back")}</button>
        {step < steps.length - 1 ? (
          <button type="button" onClick={next} className="rounded-md bg-primary px-4 py-2 text-primary-foreground">{t("btn.next")}</button>
        ) : (
          <button type="submit" className="rounded-md bg-primary px-4 py-2 text-primary-foreground">{t("btn.submit")}</button>
        )}
      </div>
    </form>
  );
